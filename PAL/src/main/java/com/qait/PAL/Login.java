...
public class Login {
	...
	public Response login(@FormParam("email") String email,@FormParam("password") String password) throws URISyntaxException{
		try{  
    		...
    		Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/PAL","root","root");    
    		...
    		while(rs.next())  
    		{
    			if(email.equals(rs.getString(1)) && password.equals(rs.getString(2)))
    			{
    				...
    				URI location = new URI("http://localhost:8080/PAL/home.html");
		...
		URI location = new URI("http://localhost:8080/PAL/login.html");
    	return Response.seeOther(location).build();
    	}
